...
our %config = (
    ...
    "ASFLAGS" => [],
    "CC" => "/workspace/musl-install/bin/musl-gcc",
    "CFLAGS" => [
        "-Wall -O3"
    ],
    "CPPDEFINES" => [],
    ...
    "LD" => "/workspace/musl-install/syslibdir/ld-musl-aarch64.so.1",
    "LDFLAGS" => [],
    "LDLIBS" => [],
    "OBJCOPY" => "objcopy",
    "PERL" => "/usr/bin/perl",
    ...
    "options" => "--prefix=/workspace/Native/opt --openssldir=/workspace/Native/usr/local --with-rand-seed=getrandom no-acvp-tests no-apps no-asan no-async no-brotli no-brotli-dynamic no-buildtest-c++ no-comp no-crypto-mdebug no-crypto-mdebug-backtrace no-demos no-devcryptoeng no-docs no-dynamic-engine no-ec2m no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fips no-fips-post no-fips-securitychecks no-fuzz-afl no-fuzz-libfuzzer no-h3demo no-idea no-jitter no-ktls no-loadereng no-md2 no-mdc2 no-msan no-pie no-rc5 no-sctp no-seed no-shared no-ssl3 no-ssl3-method no-tests no-tfo no-trace no-ubsan no-unit-test no-uplink no-weak-ssl-ciphers no-winstore no-zlib no-zlib-dynamic no-zstd no-zstd-dynamic",
    "patch" => "1",
...
unless (caller) {
    ...
    use lib '/workspace/Native/openssl-openssl-3.4.1/util/perl';
    use OpenSSL::fallback '/workspace/Native/openssl-openssl-3.4.1/external/perl/MODULES.txt';
    ...
    if (scalar @ARGV == 0) {
        ...
        use lib '.';
        use lib '/workspace/Native/openssl-openssl-3.4.1/Configurations';
        use gentemplate;
