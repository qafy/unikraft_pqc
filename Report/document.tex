...
To build OpenSSL we first have to configure it via a perl Configure script. This not only contains the configuration for the main library but also the configuragion for a large amount of tests, demos, documentation, and applications. This Configure script then generates a Makefile. Initially we tried to include OpenSSL by rewriting the Makefile to work with the Unikraft build system, though this quickly turned out to be quite difficult, because of the size of the Makfile and the fact that most of it is generated automatically. This would also mean that if we tried to update the library we potentially would have to rewrite the solution again. A proper solution for this problem would be to write create a new target system in the Configuration script. However because of the lack of time and Perl knowledge we pursued another approach. Fortunately the Unikraft build system also has the ability to inlucde prebuilt object files and static libraries. We built a static library and then used this feature to include it in Unikrafts build process. 
