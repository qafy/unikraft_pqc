...
static EVP_PKEY *oqstest_make_key(const char *type, EVP_PKEY *template,
    ...
    if (!alg_is_enabled(type)) {
        uk_pr_err("Not generating key for disabled algorithm %s.\n", type);
        return NULL;
    }
...
static int encode_EVP_PKEY_prov(const EVP_PKEY *pkey, const char *format,
    ...
    if (ectx == NULL) {
        uk_pr_err("No suitable encoder found\n");
        goto end;
    }
...
static int test_oqs_encdec(const char *alg_name) {
    ...
    for (i = 0; i < nelem(test_params_list); i++) {
        ...
        if (!OBJ_sn2nid(alg_name)) {
            uk_pr_err("No OID registered for %s\n", alg_name);
            ok = -1;
            goto end;
        }
        if (!encode_EVP_PKEY_prov(pkey, test_params_list[i].format,
            ...
            uk_pr_err("Failed encoding %s", alg_name);
            goto end;
        }
        if (!decode_EVP_PKEY_prov(
                ...
                encoded->length)) {
            uk_pr_err("Failed decoding %s", alg_name);
            goto end;
        }

        if (EVP_PKEY_eq(pkey, decoded_pkey) != 1) {
            uk_pr_err("Key equality failed for %s", alg_name);
            goto end;
        }
        EVP_PKEY_free(pkey);
        pkey = NULL;
...
int main_test_provider(int argc, char *argv[]) {
    size_t i;
    ...
    if (algs) {
        ...
        uk_test_printf( cRED "  No signature algorithms found" cNORM "\n");
        ERR_print_errors_fp(stderr);
        errcnt++;
    }
    ...
    if (algs) {
        ...
        uk_test_printf( cRED "  No KEM algorithms found" cNORM "\n");
        ERR_print_errors_fp(stderr);
        errcnt++;
    }
...
UK_TESTCASE(pqc_provider_testsuite, pqc_test_case_provider)
{				                                          
    char *argv[] = {"./test_provider", "oqsprovider", "./openssl.cnf"};				  
    int ret = main_test_provider(3, argv);					  
    UK_TEST_EXPECT_SNUM_EQ(ret, EXIT_SUCCESS);			  
}														  



uk_testsuite_register(pqc_provider_testsuite, NULL);