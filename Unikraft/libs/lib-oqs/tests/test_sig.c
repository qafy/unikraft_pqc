...
static OQS_STATUS sig_test_correctness(const char *method_name) {
	...
	if (sig == NULL) {
		uk_pr_err("ERROR: OQS_SIG_new failed\n");
		goto err;
	}

	uk_test_printf("================================================================================\n");
	uk_test_printf("Sample computation for signature %s\n", sig->method_name);
	uk_test_printf("================================================================================\n");
	...
	if ((public_key == NULL) || (secret_key == NULL) || (message == NULL) || (signature == NULL)) {
		uk_pr_err("ERROR: OQS_MEM_malloc failed\n");
		goto err;
	}
	...
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_SIG_keypair failed\n");
		goto err;
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_SIG_sign failed\n");
		goto err;
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_SIG_verify failed\n");
		goto err;
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_ERROR) {
		uk_pr_err("ERROR: OQS_SIG_verify should have failed!\n");
		goto err;
	}
	...
	if (sig->sig_with_ctx_support) {
		for (size_t i = 0; i < 256; ++i) {
			...
			OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
			if (rc != OQS_SUCCESS) {
				uk_pr_err("ERROR: OQS_SIG_sign_with_ctx_str failed\n");
				goto err;
			}
			...
			OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
			if (rc != OQS_SUCCESS) {
				uk_pr_err("ERROR: OQS_SIG_verify_with_ctx_str failed\n");
				goto err;
			}
			...
			OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
			if (rc != OQS_ERROR) {
				uk_pr_err("ERROR: OQS_SIG_verify_with_ctx_str should have failed!\n");
				goto err;
			}
		}
		...
		OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
		if (rc != OQS_ERROR) {
			uk_pr_err("ERROR: OQS_SIG_sign_with_ctx_str should only support up to 256 byte contexts\n");
			goto err;
		}
	} else {
		rc = OQS_SIG_sign_with_ctx_str(sig, signature, &signature_len, message, message_len, ctx, 1, secret_key);
		if (rc != OQS_ERROR) {
			uk_pr_err("ERROR: OQS_SIG_sign_with_ctx_str should fail without support for context strings\n");
			goto err;
		}
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_SIG_sign_with_ctx_str should always succeed when providing a NULL context string\n");
		goto err;
	}
	OQS_TEST_CT_DECLASSIFY(public_key, sig->length_public_key);
	OQS_TEST_CT_DECLASSIFY(signature, signature_len);
	...
	if (rv) {
		uk_pr_err("ERROR: Magic numbers do not match\n");
		goto err;
	}
#endif

	uk_test_printf("verification passes as expected\n");
	ret = OQS_SUCCESS;
	goto cleanup;
...
int main_test_sig(int argc, char **argv) {
	OQS_init();

	uk_test_printf("Testing signature algorithms using liboqs version %s\n", OQS_version());

	if (argc != 2) {
		uk_pr_err("Usage: test_sig algname\n");
		uk_pr_err("  algname: ");
		for (size_t i = 0; i < OQS_SIG_algs_length; i++) {
			if (i > 0) {
				uk_pr_err(", ");
			}
			uk_pr_err("%s", OQS_SIG_alg_identifier(i));
		}
		uk_pr_err("\n");
		OQS_destroy();
		return EXIT_FAILURE;
	...
	if (!OQS_SIG_alg_is_enabled(alg_name)) {
		uk_test_printf("Signature algorithm %s not enabled!\n", alg_name);
		OQS_destroy();
		return EXIT_FAILURE;
	}
...
// 		if (trc) {
// 			uk_pr_err("ERROR: Creating pthread\n");
// 			OQS_destroy();
// 			return EXIT_FAILURE;
// 		}
...
#define CREATE_SIG_TESTCASE_WITH_CIPHER(cipher, testname) \
	UK_TESTCASE(pqc_sig_testsuite, pqc_test_case_ ## testname)\
	{				                                          \
		char *argv[] = {"./test_sig", cipher};				  \
		int ret = main_test_sig(2, argv);					  \
