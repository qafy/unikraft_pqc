...
static void OQS_print_hex_string(const char *label, const uint8_t *str, size_t len) {
	uk_test_printf("%-20s (%4zu bytes):  ", label, len);
	for (size_t i = 0; i < (len); i++) {
		uk_test_printf("%02X", str[i]);
	}
	uk_test_printf("\n");
}
...
static bool mlkem_rej_testcase(OQS_KEM *kem, uint8_t *ciphertext, uint8_t *secret_key) {
	...
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_KEM_decaps failed for rejection testcase scenario 1\n");
		goto cleanup;
	}
	...
	if (rv != 0) {
		uk_pr_err("ERROR: shared secrets are not equal for rejection key in decapsulation scenario 1\n");
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_KEM_decaps failed for rejection testcase scenario 2\n");
		goto cleanup;
	}
	OQS_TEST_CT_DECLASSIFY(shared_secret_d, MLKEM_SECRET_LEN);
	OQS_TEST_CT_DECLASSIFY(shared_secret_r, MLKEM_SECRET_LEN);
	rv = memcmp(shared_secret_d, shared_secret_r, MLKEM_SECRET_LEN);
	if (rv != 0) {
		uk_pr_err("ERROR: shared secrets are not equal for rejection key in decapsulation scenario 2\n");
		OQS_print_hex_string("shared_secret_d", shared_secret_d, MLKEM_SECRET_LEN);
		OQS_print_hex_string("shared_secret_r", shared_secret_r, MLKEM_SECRET_LEN);
		goto cleanup;
	}
...
static OQS_STATUS kem_test_correctness(const char *method_name) {
	...
	if (kem == NULL) {
		uk_pr_err("ERROR: OQS_KEM_new failed\n");
		goto err;
	}

	uk_test_printf("================================================================================\n");
	uk_test_printf("Sample computation for KEM %s\n", kem->method_name);
	uk_test_printf("================================================================================\n");
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_KEM_keypair failed\n");
		goto err;
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_KEM_encaps failed\n");
		goto err;
	}
	...
	OQS_TEST_CT_DECLASSIFY(&rc, sizeof rc);
	if (rc != OQS_SUCCESS) {
		uk_pr_err("ERROR: OQS_KEM_decaps failed\n");
		goto err;
	}
	...
	if (rv != 0) {
		uk_pr_err("ERROR: shared secrets are not equal\n");
		...
		//uk_test_printf("shared secrets are equal\n");
	}
	...
	if (rc == OQS_SUCCESS && memcmp(shared_secret_e, shared_secret_d, kem->length_shared_secret) == 0) {
		uk_pr_err("ERROR: OQS_KEM_decaps succeeded on wrong input\n");
		goto err;
	}
...
int main_test_kem(int argc, char **argv) {
	OQS_init();

	uk_test_printf("Testing KEM algorithms using liboqs version %s\n", OQS_version());

	if (argc != 2) {
		uk_pr_err("Usage: test_kem algname\n");
		uk_pr_err("  algname: ");
		for (size_t i = 0; i < OQS_KEM_algs_length; i++) {
			if (i > 0) {
				uk_pr_err(", ");
			}
			uk_pr_err("%s", OQS_KEM_alg_identifier(i));
		}
		uk_pr_err("\n");
		OQS_destroy();
		return EXIT_FAILURE;
	...
	if (!OQS_KEM_alg_is_enabled(alg_name)) {
		uk_test_printf("KEM algorithm %s not enabled!\n", alg_name);
		OQS_destroy();
		return EXIT_FAILURE;
	}
...
#define CREATE_KEM_TESTCASE_WITH_CIPHER(cipher, testname) \
UK_TESTCASE(pqc_kem_testsuite, pqc_test_case_ ## testname)\
{				                                          \
	char *argv[] = {"./test_kem", cipher};				  \
